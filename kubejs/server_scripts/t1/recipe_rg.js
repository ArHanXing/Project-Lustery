...
CustomMachineryEvents.recipeFunction("t1.rg.generate", e => {
    ...
    //console.log(`[RG/BlockPlace] 开始在位置 ${getXYZ(targetPos)} 放置方块`);
    for (let xOffset = -1; xOffset <= 1; xOffset++) {
        for (let zOffset = -1; zOffset <= 1; zOffset++) {
            var placePos = targetPos.offset(xOffset, 0, zOffset);
            //console.log(`[RG/BlockPlace] 尝试放置方块！ at ${getXYZ(placePos)}`);
            try {
                world.setBlock(placePos, riftBlock, 3);
            } catch (error) {
                //console.error(`[RG/BlockPlace]放置方块失败！ at ${getXYZ(placePos)}: ${error}`);
            }
        }
    }
    //console.log(`[RG/BlockPlace] 成功放置了 ${count} 个方块`);
    e.success();
})